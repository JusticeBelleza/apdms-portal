...
import { Bell, Trash2 } from "lucide-react"; // Import Trash2 for the clear icon
  ...
  useEffect(() => {
    const handleClickOutside = (event) => {
      if (
        ...
        !event.target.closest("button.relative.p-2")
      ) {
        onClose();
      }
    };
    ...
    >
      ...
      <div className="space-y-2 max-h-80 overflow-y-auto p-3">
        {notifications.map((notif) => (
          <div
            key={notif.id}
            className={`p-3 rounded-lg relative hover:bg-gray-100 ${
